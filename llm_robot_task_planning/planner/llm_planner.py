...
class LLMTaskPlanner:
        ...
        if api_key and api_key.startswith("sk-"):
            try:
                ...
                prompt = (
                    ...
                    f"If the instruction mentions multiple locations in sequence, use a single 'follow_waypoints' skill with a list of waypoints, e.g. [{{'skill': 'follow_waypoints', 'arguments': {{'waypoints': [{{'x': -1.4, 'y': -1.2, 'theta': 3.14}}, {{'x': 1.4, 'y': -1.2, 'theta': 0.0}}]}}}}]"
                )
    ...
    def _smart_mock_plan(self, text: str) -> list[dict]:
        ...
        # If multiple locations, use follow_waypoints
        if len(visited) > 1:
            waypoints = []
            for loc_name in visited:
                loc = self.locations[loc_name]
                waypoints.append({"x": loc['x'], "y": loc['y'], "theta": loc['theta']})
            plan.append({
        ...
        # If "stop" appears, add a stop skill
