...
class NavigationNode(Node):
    def __init__(self):
        ...
        # Robot State in ENU coordinates
        self.current_x = 0.0
        self.current_y = -1.7
        ...
        # 20 Hz Real-Time Control Loop Timer
    ...
    def execute_skill_callback(self, msg: String):
        ...
        if skill == "navigate_to":
            ...
            self.waypoints_queue = list(args.get('waypoints', []))
            if self.waypoints_queue:
                ...
                self.publish_result(SkillResult(skill="follow_waypoints", success=True, reason="empty_waypoints", position_error_m=0.0, elapsed_time_s=0.0))
        elif skill == "stop":
            self.is_navigating = False
            self.cmd_vel_pub.publish(Twist())
            self.publish_result(SkillResult(skill="stop", success=True, reason="stopped", position_error_m=0.0, elapsed_time_s=0.0))
        else:
            self.publish_result(SkillResult(skill=skill, success=False, reason="Unknown skill", position_error_m=0.0, elapsed_time_s=0.0))
    ...
    def control_loop(self):
        ...
        if elapsed > self.timeout_s:
            ...
            self.publish_result(SkillResult(skill=self.current_skill_name, success=False, reason="timeout",
                                           position_error_m=round(err, 3), elapsed_time_s=round(elapsed, 2)))
            return
        ...
        POSITION_TOLERANCE = 0.25 # 25 cm (well within 80cm pad)
        ...
        if dist_err > POSITION_TOLERANCE:
            ...
            if abs(final_head_err) > HEADING_TOLERANCE:
                ...
                if self.waypoints_queue:
                    ...
                    self.start_navigate_to(next_wp['x'], next_wp['y'], next_wp.get('theta', 0.0), skill_name="follow_waypoints")
                else:
                    self.is_navigating = False
                    self.publish_result(SkillResult(skill=self.current_skill_name, success=True, reason="arrived",
                                                   position_error_m=round(dist_err, 3), elapsed_time_s=round(elapsed, 2)))
