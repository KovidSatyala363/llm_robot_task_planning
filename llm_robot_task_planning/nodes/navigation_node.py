...
class NavigationNode(Node):
    def __init__(self):
        ...
        # Waypoints queue for follow_waypoints
    ...
    def control_loop(self):
        ...
        if elapsed > self.timeout_s:
            ...
            # If this was part of follow_waypoints, clear remaining waypoints and report failure
            if self.current_skill_name == "follow_waypoints":
                self.waypoints_queue.clear()
                self.publish_result(SkillResult(
                    skill="follow_waypoints",
                    success=False,
                    reason="timeout at waypoint",
        ...
        if dist_err > POSITION_TOLERANCE:
            ...
            if abs(final_head_err) > HEADING_TOLERANCE:
                ...
                # If we are in follow_waypoints and there are more waypoints
                if self.current_skill_name == "follow_waypoints" and self.waypoints_queue:
                    ...
                    self.is_navigating = False
                    # Determine overall elapsed time for follow_waypoints if applicable
                    if self.current_skill_name == "follow_waypoints":
